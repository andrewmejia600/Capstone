...
#DCAD_aprl_year %>% filter(ACCOUNT_NUM == '00DART0000000000')

#All DCAD Account owner addresses 
DCAD_addresses = read.table(unz("DCAD2019_CURRENT.ZIP", "account_info.csv"), sep = ",", header = TRUE)
...
###337 ambigous parcels observations, 34 total Parcels 
ambigious_filtered_parcels = c(unique(DCAD_aprl_year_filtered_by_clipped_parcels[duplicated(DCAD_aprl_year_filtered_by_clipped_parcels$GIS_PARCEL_ID),43]))

### 336 ambigous accts observations, 33 total Parcels 
ambigious_filtered_accts = c(unique(DCAD_aprl_year_filtered_by_clipped_parcels[duplicated(DCAD_aprl_year_filtered_by_clipped_parcels$Acct),2]))

#ACCT 00000808916600000 is the only Parcel ID that has 2 accounts not rail 
filtered_ambiguity_overlap = setdiff(ambigious_filtered_parcels, ambigious_filtered_accts)

##Note there is not an address only PO box for ambigous accounts owned by the city of dallas or 1 address for an enitre parcel that spans multiple shapes look at '00DART00000000000'
DCAD_addresses %>% filter(ACCOUNT_NUM %in% ambigious_filtered_accts)

DCAD_addresses %>% filter(ACCOUNT_NUM %in% '00DART00000000000')
...
##There are about 30K ambiguous parcels 
ambigious_parcels = c(unique(DCAD_aprl_year[duplicated(DCAD_aprl_year$GIS_PARCEL_ID),39]))

### We see in DCAD_Apprl year there are unique ACCT numbers 
ambigious_accts_dcad = c(unique(DCAD_aprl_year[duplicated(DCAD_aprl_year$ACCOUNT_NUM),1]))
...
#CPAL labeled this entire instance as vacant, spans multiple polygons
CPAL_Label_annotations %>% filter(ACCOUNT_NUM %in% '00DART00000000000')
...
###CPAL CLAIMS Labels from Vac Parcels More Accurate But look at 00DALAREARAPTDA00
CPAL_Par_annotation = read.csv(file=paste0(file_direct,'Clipped_Parcels_by_Dallas_Simple_DCAD_Parcels_annotations.csv'))
#CPAL_Par_annotation %>% filter(CPAL_Par_annotation$ACCOUNT_NUM == "00DALAREARAPTDA00")
...
ambigious_accounts = c(unique(data[duplicated(data$Acct.x.u),5]))

dim(DCAD_test_set %>% filter(Acct.x.z %in% ambigious_accounts))[1]

data = data[!duplicated(data$Acct.x.u),]

dim(data)[1]

## only 2.39% of data is ambiguous 
(dim(DCAD_test_set %>% filter(Acct.x.z %in% ambigious_accounts))[1]/dim(data)[1])*100

### Look at their perceived better account annotations, many of these ambigouus accounts have muliple entries. Bias in Labeling data. 
CPAL_Par_annotation %>% filter(CPAL_Par_annotation$ACCOUNT_NUM %in% ambigious_accounts)
