...
```
```{r}
data["VAC_PAR_T"] = as.factor(data$VAC_PAR_T)
rand_seed = 42
set.seed(rand_seed)
train_partition =  createDataPartition(
  y= data$VAC_PAR_T,
...
#make the prediction
pred = predict(xgbFinal, data.matrix(test[,c(1:3)]))

preds_1_cut = ifelse(pred>.5,1,0)
confusionMatrix(as.factor(preds_1_cut),as.factor(test$VAC_PAR), positive = "1")
F_meas(as.factor(preds_1_cut),as.factor(test$VAC_PAR))
...
```{r}
#generage ROC curve
myPred = prediction(pred,test$VAC_PAR)
...
pred_label = as.numeric(pred>.50)
#create confusion matrix
confusionMatrix(factor(pred_label), factor(test$VAC_PAR) , positive = "1")
```
