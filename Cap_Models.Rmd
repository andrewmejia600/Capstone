...
xgbpred_best_cut =  ifelse(xgbpred_best > 0.5,1,0)
```

```{r}
confusionMatrix(as.factor(xgbpred_best_cut), as.factor(data_test_l))
F_meas(as.factor(xgbpred_best_cut),as.factor(data_test_l))
```

```{r}
xgb.importance(feature_names = colnames(train[,-1]), model = xgboost_best, data=train[,-1], label=train[,1])
```

```{r}
xgb.plot.importance(xgb.importance(feature_names = colnames(train[,-1]), model = xgboost_best, data=train[,-1], label=train[,1]), top_n = 10)
```
