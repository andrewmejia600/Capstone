...
```{r}
# Create a column for type of permit for most recent permit

building_permits = read.csv("Data/CPAL/building_permits.csv")

building_permits %>%
  select(Address, X, Y, Permit_Type, PermitDate, Mapsco) %>%
  mutate(PermitDate = as.Date(substr(PermitDate, 1, 10))) %>%
  group_by(Address, X, Y) %>%
  filter(PermitDate == max(PermitDate)) %>%
  mutate(Days_Since_Permit =  Sys.Date() - PermitDate)
```
