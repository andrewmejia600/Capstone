...
read_data = read.csv('https://raw.githubusercontent.com/andrewmejia600/Capstone/Laura_models/Data/df_log_and_scaled.csv')
#data = do.call(data.frame,lapply(read_data, function(x) replace(x, is.infinite(x),0)))

data = read_data
colnames(data)[1] = "VAC_PAR"
#data = data %>% select(-num_sptd)  # edit here to change factors in different scenarios
...
F_meas = F_meas(as.factor(preds_1_cut),as.factor(test$VAC_PAR))
precision = precision(as.factor(preds_1_cut),as.factor(test$VAC_PAR))
recall = recall(as.factor(preds_1_cut),as.factor(test$VAC_PAR))
set.seed(rand_seed)
...
set.seed(rand_seed)
varImpPlot(random_forest_1_best, cex = .7, main = "Best Variable Importance",pt.cex = 1,color = 'grey41',frame.plot = FALSE,lcolor = 'black')
...
write.csv(test, 'RF_test_out_base.csv')